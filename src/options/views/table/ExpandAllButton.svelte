<script lang="ts">
  let { table } = $props();
  // Corrigir: expanded precisa ser reativo ao estado da tabela
  let expanded = $derived(() => table.options.state.expanded);
  function toggleAll() {
    const allExpanded = Object.values(expanded()).some(Boolean);
    if (allExpanded) {
      table.options.onExpandedChange?.({});
    } else {
      const all = Object.fromEntries(table.getRowModel().rows.map((row: any) => [row.id, true]));
