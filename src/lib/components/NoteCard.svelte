...
<div class="note-card p-4 rounded-lg shadow-sm transition-all" style="background-color: {note?.color || '#f9fafb'}">
  ...
  {:else if isEditing}
    <div class="note-edit-form space-y-3">
      ...
      <div>
        <label for="note-tags-{note.id}" class="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">
          Tags
        </label>
        <input 
        ...
        <div class="flex flex-wrap gap-2" role="radiogroup">
          {#each colorOptions as option}
            <button 
              ...
              tabindex="0"
              class="color-option w-6 h-6 rounded-full cursor-pointer border-2 {editedColor === option.value ? 'border-blue-500 ring-2 ring-blue-300' : 'border-gray-300 dark:border-gray-600'}"
